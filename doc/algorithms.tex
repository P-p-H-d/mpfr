...
\paragraph{\textbf{Implementation.}} The code does not compute $t$ directly;
instead, it computes the following exponent bound. The leading term is
evaluated differently depending on the parity of $n$:
\begin{itemize}
\item If $n=2m$ is even, the leading term is $c_{0,n} = P_n(0)$, computed by
  \[
    c_{0,0}=1, \qquad c_{0,j}=c_{0,j-2}\,\frac{-(2j-1)}{2j},
    \quad 1 \leq j \leq m,
  \]
  thus
  \[
    c_{0,2m} = \prod_{j=1}^{m} \frac{-(2j-1)}{2j}.
  \]

\item If $n=2m+1$ is odd, the leading term is $c_1x$ with $c_{1,n}=P_n'(0)$,
  computed by
  \[
    c_{1,0}=1, \qquad c_{1,j}=c_{1,j-2}\,\frac{-(2j+1)}{2j},
    \quad 1 \leq j \leq m,
  \]
  thus
  \[
    c_{1,2m+1} = \prod_{j=1}^{m} \frac{-(2j+1)}{2j}.
  \]
\end{itemize}
We remark that $c_{1,2m+1} = (2m+1) c_{0,2m}$.
