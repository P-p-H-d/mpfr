...
{\color{blue}
\begin{itemize}
\item If $n=2m$ is even, the leading term is $c_m^{(0)} = P_n(0)$, computed by
  \[
    c_0^{(0)}=1, \qquad c_j^{(0)}=c_{j-1}^{(0)}\,\frac{-(2j-1)}{2j},
    \quad 1 \leq j \leq m,
  \]
  thus
  \[
    c_m^{(0)} = \prod_{j=1}^{m} \frac{-(2j-1)}{2j}.
  \]

\item If $n=2m+1$ is odd, the leading term is $c_m^{(1)} x$ with
  $c_m^{(1)}=P_n'(0)$, computed by
  \[
    c_0^{(1)}=1, \qquad c_j^{(1)}=c_{j-1}^{(1)}\,\frac{-(2j+1)}{2j},
    \quad 1 \leq j \leq m,
  \]
  thus
  \[
    c_m^{(1)} = \prod_{j=1}^{m} \frac{-(2j+1)}{2j}.
  \]
\end{itemize}
}
...
\paragraph{\textbf{Error analysis of $c_{0,2m}$ and $c_{0,2m+1}$}}
...
precision $p$. Each \texttt{mpfr\_mul\_si} and \texttt{mpfr\_div\_ui} call
rounds the exact arithmetic result, introducing a relative error of at most
$u = 2^{-p}$ per operation. Denote by $V_k$ the exact real value of the
...
\[
  ...
  \le 2ku = k \cdot 2^{1-p},
\]
where the first inequality holds by induction (each operation multiplies the
relative discrepancy by at most $(1+u)$), and the last holds whenever
$ku \le \tfrac{1}{2}$, i.e. $p \ge 1 + \lceil\log_2 k\rceil$, which is
satisfied by our choice of working precision.
